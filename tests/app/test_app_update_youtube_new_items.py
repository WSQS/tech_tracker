...
"""
    ...
    assert new_item_ids == {"youtube:abc123def456", "youtube:xyz789uvw012"}
    
    # Verify store now contains 2 items
    stored_items = store.load_all()
    assert len(stored_items) == 2
    stored_item_ids = {item.item_id for item in stored_items}
    assert stored_item_ids == {"youtube:abc123def456", "youtube:xyz789uvw012"}
...
"""
    ...
    assert new_items_second[0].item_id == "youtube:new789xyz456"
    ...
    stored_item_ids = {item.item_id for item in stored_items}
    assert stored_item_ids == {"youtube:abc123def456", "youtube:xyz789uvw012", "youtube:new789xyz456"}
    
    # Verify the new item is the latest (sorted by published time)
    latest_item = stored_items[0]  # First item should be latest
    assert latest_item.item_id == "youtube:new789xyz456"
